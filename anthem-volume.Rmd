...
date_next_14days <- format(Sys.Date() +12, "%Y-%m-%d")
...
date_end_2025 <- format(as.Date(paste0("2025-",format(Sys.Date()-1, "%m-%d"))), "%Y-%m-%d")

date_start_2026 <- format(as.Date("2026-01-05"), "%Y-%m-%d")
date_end_2026 <- format(Sys.Date()-2, "%Y-%m-%d")
...
# flag2 <- tbl(conn2, "PATIENT_FYI_FLAGS") %>%
#   group_by(PATIENT_ID) %>%
#   filter(LAST_UPDATE_INST == max(LAST_UPDATE_INST, na.rm = TRUE)) %>%
#   collect()
