...
mtd_start_2025 <- date_end_2025 - (num_days - 1)
...
mtd_volume_comparison_group_summary <- mtd_volume_comparison2_group %>%
  group_by(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, 
           appt_year, pat_group) %>%
  summarise(total = sum(total, na.rm = T))


mtd_volume_comparison_group_summary2 <- mtd_volume_comparison_group_summary %>%
  pivot_wider(names_from = c("pat_group","appt_year"),
              values_from = total) %>%
  mutate(Total_2025 = rowSums(across(contains("2025")), na.rm = TRUE),
         Total_2026 = rowSums(across(contains("2026")), na.rm = TRUE)) %>%
...
mtd_appt_day_comparison <- mtd_volume_comparison2_group %>%
  group_by(appt_year, appt_day) %>%
  summarise(total = n_distinct(CONTACT_DATE)) %>%
  pivot_wider(names_from = appt_day,
              values_from = total)
...
no_show <- access_raw_tbl %>%
  ...
  group_by(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, PROV_TYPE, PROV_ID, PROV_NAME_WID, DERIVED_STATUS_DESC,
           # PAT_ENC_CSN_ID, PAT_NAME, MRN, APPT_DTTM, VISITPLAN,
...
check <- slot_merged %>%
  ...
  group_by(slot_date) %>%
  summarise(arrived = sum(ARRIVED_MIN, na.rm = T)) %>%
  collect()
...
## No Show by Week
```{r No Show and Fill Rate by Week, echo = FALSE, warning = FALSE, message = FALSE}
...
htmltools::browsable(
  tagList(
    tags$button(
      tagList(fontawesome::fa("download"), "Download Table"),
      onclick = "Reactable.downloadDataCSV('no-show-template-util-week', 'No Show and Fill Rates')"
    ),
...
reactable(
  ...
  elementId = "no-show-template-util-week",
  # elementId = "referral-vol-download-table",
  style = list(fontFamily = 'Calibri',
                 fontSize = '14px'),
    defaultColDef = colDef(align = "left",
  ...
  columns = list(
     ...
     MSS_Attributed_NS_SDC_Rate = colDef(
      ...
      style = JS("function(rowInfo) {
        var value = rowInfo.row['MSS_Attributed_NS_SDC_Rate']
        if (value > 0) {
          var color = '#e00000'
        } else {
          var color = '#008000'
...
htmltools::browsable(
  tagList(
    tags$button(
      tagList(fontawesome::fa("download"), "Download Table"),
      onclick = "Reactable.downloadDataCSV('no-show-template-util-site', 'No Show and Fill Rates')"
    ),
...
reactable(
  merged %>%
    ...
    filter(SITE %in% c("MSBI", "MSDMG", "MSH- AMBULATORY CARE", "MSH-MSDFP", "MSM", "MSQ",               
                       "MSSN",  "MSUS", "MSW", "NETWORK", "NYEE", "MSB")),
  elementId = "no-show-template-util-site",
  # elementId = "referral-vol-download-table",
  style = list(fontFamily = 'Calibri',
                 fontSize = '14px'),
    defaultColDef = colDef(align = "left",
  ...
  groupBy = c("MSHS", "SITE", "appt_week", "DEPT_SPECIALTY_NAME", "DEPARTMENT_NAME", "PROV_TYPE"),

  columnGroups = list(
    colGroup(name = "", columns = c("MSHS", "SITE", "appt_week", "DEPT_SPECIALTY_NAME", "DEPARTMENT_NAME", "DEPARTMENT_ID",
                                    "PROV_TYPE", "PROV_NAME_WID"),
             headerStyle = list(color = "black", fontSize = "15px"),
             sticky = "left"),
    colGroup(name = paste0("2026 Completed Visits (", format(as.Date("2026-01-01"), "%m/%d"),"-",format(as.Date(slot_date_start), "%m/%d"),")"),
